...
class SevenSeries(XilinxPLL, core, gekko_translation):
    ...
    transceiver_types_available = ["GTXE2", "GTHE2"]  # We don't support GTHE2 yet!
...
class CPLL(PLLCommon):
    ...
    def add_constraints(self, config: dict, fpga_ref, converter) -> dict:
        ...
        if v == [0, 1]:
            self.model.add_kpi(
                config[converter.name + "_use_cpll"], name=converter.name + "_use_cpll"
            )
        # Add variables
...
class QPLL(PLLCommon):
    ...
    def add_constraints(self, config: dict, fpga_ref, converter) -> dict:
        ...
        if v == [0, 1]:
            self.model.add_kpi(
                config[converter.name + "_use_qpll"], name=converter.name + "_use_qpll"
            )
        ...
        if self.parent.transceiver_type[:3] == "GTH":
            ...
            # See page 72 of https://docs.amd.com/v/u/en-US/ds191-XC7Z030-XC7Z045-data-sheet
            c5 = if_then(
                config[converter.name + "_use_qpll"] == 1,
                if_then(
                    config[converter.name + "_lower_band_qpll"] == 0,
