...
# AD9371 TX deframer requires L in {2, 4} and Np = 16.
quick_configuration_modes_tx = {
    # M = 1
    str(0): _convert_to_config(M=1, L=2, Np=16),
    # M = 2
    str(1): _convert_to_config(M=2, L=2, Np=16),
    str(2): _convert_to_config(M=2, L=4, Np=16),
    # M = 4
    str(3): _convert_to_config(M=4, L=2, Np=16),
    str(4): _convert_to_config(M=4, L=4, Np=16),
}


# AD9371 RX framer supports Np in {12, 16} and L in {1, 2, 4}.
...
def _extra_jesd_check(dev: converter) -> None:
    ...
    assert 17 <= dev.K <= 32, "K must be in [17, 32]"
