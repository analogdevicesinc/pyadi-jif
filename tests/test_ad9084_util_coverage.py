...
import adijif
from adijif.converters.ad9084_util import (
    _convert_to_config,
    apply_settings,
    parse_json_config,
)
...
def test_ad9084_util_convert_to_config():
    ...
    res = _convert_to_config(
        "1", 1, 2, 3, 4, 1, 32, 16, 16, 0, 0, 0, "jesd204c"
    )
    assert res["L"] == 1
    assert res["M"] == 2
...
def test_ad9084_util_parse_json_config_file_not_found():
    ...
    with pytest.raises(
        FileNotFoundError, match="Profile JSON file does not exist"
    ):
...
def test_ad9084_util_parse_json_config_invalid_version():
    ...
    data = {
        "profile_cfg": {"profile_version": {"major": 0, "minor": 0, "patch": 0}}
    }
...
def test_ad9084_util_apply_settings_mode_mismatch():
    ...
    with pytest.raises(
        ValueError, match="TX and RX JESD204C modes do not match"
    ):
